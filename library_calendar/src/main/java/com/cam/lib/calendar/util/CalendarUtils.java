...
public class CalendarUtils {
    ...
    public static int[] getLastMonthDay(int year, int month, int day){
        int[] arr = new int[]{year, month, day};
        if(arr[1] == 0){
            arr[0] --;
            arr[1] = 11;
        }else{
        ...
        int monthDay = getMonthDayNum(arr[0], arr[1]);
        if(day > monthDay){
            arr[2] = monthDay;
        }
        return arr;
    ...
    public static int[] getNextMonthDay(int year, int month, int day){
        int[] arr = new int[]{year, month, day};
        if(arr[1] == 11){
            arr[0] ++;
            arr[1] = 0;
    ...
    public static int[] getLastWeekDay(int year, int month, int day){
        day -= 7;
        if(day <= 0){
            if(month == 0){
                year --;
            ...
            day += getMonthDayNum(year, month);

        }
        return new int[]{year, month, day};
    }

    public static int[] getNextWeekDay(int year, int month, int day){
        day += 7;
        int monthDay = getMonthDayNum(year, month);
        if(day > monthDay){
            day -= monthDay;
            if(month < 12){
                month ++;
